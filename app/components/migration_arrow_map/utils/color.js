...
export default function quantizeColorScaleFactory(
  ...
  const colorDivs = colorScale.range().map(c => `<div title="${c}" style="flex: 1 1 auto; height: ${height}px; background: ${c};"></div>`);

  const ticks = colorScale.range().filter((d, i, a) => i < a.length - 1).map((c, i) => `<div style="
          font-family: Trebuchet MS, Helvetica, sans-serif;
          font-size: 20px;"
        >${format(tickFormat)(scale.invert(colorScale.invertExtent(c)[1]))}</div>`);
